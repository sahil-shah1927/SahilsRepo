...
<%@ page import="com.fdmgroup.heatseeker.model.Department"%>
...
<%
	...
	try {
		...
		myIssues = issueDAO.readAll();
		allDepartments = deptDAO.readAll();
...
<table
	...
	<tbody>
		...
		<c:choose>
										...
										<form class="addIssueUpdateForm" action="addIssueUpdate"
											...
											<div class="form-group inline-block issue-update-input">
												<input type="hidden" name="issueId"
													...
													placeholder="Add an update.." name="updateText"
													maxlength="300" required>
											</div>
											<div class="form-group inline-block">
												<button
													class="addIssueUpdateButton center-block btn btn-hover-lightblue"
													type="submit" name="addUpdate">Submit</button>
											</div>
										</form>
									</div>
								</div>

								<c:if test='${currentIssue.department.deptName == "General"}'>
									<div
										class="issue-department-update-container center-block center-text">
										<form class="changeDepartmentForm" action="changeDept"
											method="POST" class="issue-details-update-dept">
											<input type="hidden" name="issueId"
												value="${currentIssue.issueId}" /> <select
												class="issue-dept-select form-control inline-block"
												required="required" name="deptId">
												<option value="" disabled selected>Change
													Department</option>
												<c:forEach var="currentDept" items="<%=allDepartments%>">
													<c:if test='${currentDept.deptName != "General"}'>

														<option value="${currentDept.deptId}">${currentDept.deptName}</option>
													</c:if>

												</c:forEach>
											</select>
											<button
												class="changeDepartmentButton btn inline-block btn-hover-lightblue"
			...
			<c:otherwise>
				<c:if test='${statusOfTab == "New" }'>
					<%
						myIssues = deptDAO.readAllIssuesByDept(admin.getDept().getDeptId());
					%>
				</c:if>
				<c:forEach var="currentIssue" items="<%=myIssues%>">
									...
									<div class="issue-updates-container center-block">
										...
										<div
											...
											<form class="addIssueUpdateForm" action="addIssueUpdate"
												...
												<div class="form-group inline-block issue-update-input">
													<input type="hidden" name="issueId"
														value="${currentIssue.issueId}" /> <input
														class="form-control update-textarea width-100" type="text"
														placeholder="Add an update.." name="updateText"
														maxlength="300" required>
												</div>
												<div class="form-group inline-block">
